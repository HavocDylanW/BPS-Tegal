...
class SubmissionController extends Controller
{
    ...
    public function edit(Submission $submission)
    {
        ...
        // Restrict editing if the submission is approved
        if ($submission->isApproved()) {
            return redirect()->route('assignments.index')->with('error', 'Approved submissions cannot be edited.');
        }
    ...
    public function update(Request $request, Submission $submission)
    {
        ...
        // Restrict updating if the submission is approved
        if ($submission->isApproved()) {
            return redirect()->route('assignments.index')->with('error', 'Approved submissions cannot be updated.');
        }
    ...
    {
        if ($submission->isApproved()) {
            return redirect()->back()->with('error', 'Submission is already approved.');
        }

        $submission->update(['approval_status' => 1]); // 1 = Approved
    ...
    public function reject(Submission $submission)
    {
        if ($submission->isRejected()) {
            return redirect()->back()->with('error', 'Submission is already rejected.');
        }

        $submission->update(['approval_status' => 2]); // 2 = Rejected
